...
var commentRequests=0;$.ajax=opts=>{const d=$.Deferred();const comments=opts.url.includes('/actions');if(comments)commentRequests++;setTimeout(()=>d.resolve(comments?(opts.url.includes('/cards/a/')?[{id:'comment',data:{text:'# Tech Design\\n## Test Cases\\n### Branch'}}]:[]):boardData),5);return d.promise()};
...
el('[data-check="missingComments"]').click();el('[data-check="missingComments"]').click();await wait(20);check(commentRequests===3,'repeat toggle reuses comment data');el('[data-check="missingComments"]').click();
